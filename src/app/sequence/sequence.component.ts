...
export class SequenceComponent implements OnInit {
    ...
    private full$ = this.panicForm.valueChanges.pipe(startWith(this.panicForm.value));

    private resRough$ = this.full$.pipe(
        map((fullForm): boolean[] => {
    ...
    private dmg() {
        return combineLatest([this.resRough$, this.full$]).pipe(
            map(([results, form]): number => {
                const successes = results.filter((r) => r).length;
                const damagePerFail = (form.panicked ? this.dmgAvgPanicked : this.dmgAvg) + form.extraDamagePerFail;
